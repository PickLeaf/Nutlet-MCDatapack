...
```
会在执行位置summon一个方块展示实体(为钻石块)，并在100tick后消失，transformation标签会应用到展示实体的transformation标签。定时消失的功能由nutlet:-m/schedule实现。执行实体推荐为玩家，这会使得清除展示实体时自动/forceload强加在(/execute if loaded条件为通过时)其所在区块，如果不是玩家可能会因为展示实体所在的区块被卸载而无法被清除(这种情况退出存档重进即可)。
```mcfunction
